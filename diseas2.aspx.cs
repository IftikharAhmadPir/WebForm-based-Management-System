...
{
    public partial class diseas2 : System.Web.UI.Page
    {
        ...
        {
            string disease = ddl_disease.SelectedValue;
            lbl_result.Text = disease;

            string sql = "select * from disease where id='" + disease + "'";
            if (disease != "")
            {
                DataTable dt = search(sql);
                int c = dt.Rows.Count;
